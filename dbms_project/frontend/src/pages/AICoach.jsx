...
export default function AICoach({ apiUrl, token, user }) {
  ...
  return (
    <motion.div 
      ...
      className="h-[82vh] flex flex-col bg-[#1E1E1E] border border-[#474747]/40 rounded-3xl overflow-hidden shadow-2xl text-white"
    >
      {/* Coach Header */}
      <div className="flex items-center gap-3 px-6 py-4 border-b border-[#474747]/40 bg-[#0A0A0A]">
        <div className="h-10 w-10 rounded-2xl bg-[#D4FF00] flex items-center justify-center text-black shadow-md">
          <Bot size={22} />
        </div>
        <div>
          <h3 className="font-display text-xl font-bold text-white flex items-center gap-2 uppercase tracking-wide">
            STRYVON AI COACH
            <Sparkles size={16} className="text-[#D4FF00] animate-pulse" />
          </h3>
          <span className="text-[10px] font-display text-[#D4FF00] font-bold tracking-widest uppercase">Intelligent Personal Trainer & Dietician</span>
        </div>
      </div>

      {/* Messages Scroll Area */}
      <div className="flex-1 p-6 overflow-y-auto space-y-4 scrollbar-thin bg-[#1E1E1E]">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center max-w-md mx-auto space-y-6">
            ...
            <div>
              <h4 className="font-display text-xl font-bold text-white uppercase tracking-wide">Start Your AI Fitness Session</h4>
            ...
            <div className="grid grid-cols-2 gap-3 w-full">
              {quickPrompts.map((p, idx) => (
                <button
                  ...
                  className="p-3 text-[11px] font-display font-bold uppercase tracking-wider text-[#E5E5E5] hover:text-[#D4FF00] bg-[#0A0A0A] border border-[#474747]/40 hover:border-[#D4FF00]/50 rounded-2xl transition-all text-center leading-tight cursor-pointer"
                >
                  {p.label}
                </button>
              ))}
          ...
          <>
            {messages.map((msg, index) => (
              ...
              >
                {/* Avatar */}
                <div className={`h-8 w-8 rounded-full shrink-0 flex items-center justify-center font-bold ${msg.sender === 'user' ? 'bg-white text-black' : 'bg-[#D4FF00] text-black'}`}>
            ...
            {loading && (
              <div className="flex gap-3 mr-auto max-w-[85%]">
                <div className="h-8 w-8 rounded-full bg-[#D4FF00] flex items-center justify-center text-black">
                  <Bot size={14} />
                </div>
                <div className="bg-[#0A0A0A] border border-[#474747]/50 p-4 rounded-3xl rounded-tl-none flex items-center gap-1.5">
      ...
      <form 
        ...
        className="p-4 border-t border-[#474747]/40 bg-[#0A0A0A] flex gap-3 items-center"
      >
        <input
          type="text"
          disabled={loading}
          placeholder="Ask STRYVON coach (e.g. 'Generate a calorie plan' or 'Analyze my weight trend')..."
          value={input}
          onChange={(e) => setInput(e.target.value)}
          className="flex-1 bg-[#1E1E1E] border border-[#474747] rounded-2xl py-3 px-5 text-sm focus:outline-none focus:border-[#D4FF00] text-white placeholder:text-[#474747]"
        />
        <button
          type="submit"
          disabled={loading || !input.trim()}
          className="p-3 bg-[#D4FF00] hover:bg-[#b8de00] text-black rounded-2xl shadow-md transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
        >
          <Send size={18} />
        </button>
      </form>
